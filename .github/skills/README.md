...
| Skill | What it does | Use it when |
|---|---|---|
| [`servpro-crm-review`](./servpro-crm-review/SKILL.md) | Pulls all open jobs from Dash CRM, flags missing info and docs, identifies blockers, generates stale-job alerts, and produces Gemini AI-drafted CRM notes and Gmail follow-up drafts every 48 hours. | Running the daily/48-hour CRM triage; getting stale job alerts; drafting CRM notes and follow-up emails. |
| [`servpro-workspace-sync`](./servpro-workspace-sync/SKILL.md) | Cross-posts Dash CRM data across all Google Workspace apps: Gmail, Calendar, Tasks, Contacts, Drive, Docs, Sheets, Forms, Photos, and Chat. | Syncing Google Workspace with Dash after any CRM update; doing the periodic workspace alignment review. |
| [`servpro-pipeline-intake`](./servpro-pipeline-intake/SKILL.md) | Monitors Gmail and Google Chat for new leads, creates Drive folders and Google Contacts, generates Gemini AI outreach drafts, creates pre-filled Forms links, and enforces the stage-gate checklist before any job moves to WIP. | Receiving a new lead; setting up a new job file; checking stage-gate readiness before WIP mobilization. |
| [`servpro-ops-review`](./servpro-ops-review/SKILL.md) | Runs the Monday/Wednesday/Friday operational cadence: full calendar audit, Gemini AI crew message drafts for Google Chat, crew photo compliance enforcement (48-hour rule), midweek KPI and blocker review, Friday Sheets tracker update and PDF export, by-customer money summary, and management report. | Every Monday, Wednesday, and Friday morning; any crew accountability check; weekly tracker update. |
| [`servpro-google-sheets`](./servpro-google-sheets/SKILL.md) | Maintains all Sheets-based financial tracking: Weekly Management Accountability Tracker, A/R aging summary, collections dashboard, crew production log, and job pipeline board. Reads from Dash, writes to Sheets, exports PDF to Drive each Friday. | Weekly tracker update; financial dashboard refresh; A/R aging report; Friday PDF export. |
| [`servpro-google-chat`](./servpro-google-chat/SKILL.md) | Monitors Chat spaces for new leads and crew updates. Routes new leads to pipeline-intake, drafts crew coordination replies, and flags urgent escalations. | New lead arrives via Chat; daily crew thread triage; urgent escalation monitoring. |
| [`servpro-gemini-drafts`](./servpro-gemini-drafts/SKILL.md) | AI-powered draft generation via Gemini API. Converts job data and voice-note inputs into polished CRM notes, customer emails, adjuster letters, crew messages, and supplement summaries in Tristan's voice. | After CRM review; when converting rough voice notes; generating supplement letters. |
| [`servpro-vertex-ai`](./servpro-vertex-ai/SKILL.md) | Advanced analysis via Google Cloud Vertex AI (Gemini Enterprise). Job risk scoring, insurance scope validation, supplement approval forecasting, and Friday executive summary. Uses Workload Identity — no long-lived keys. | Weekly risk analysis; insurance scope validation; supplement forecasting; Friday executive summary. |
| [`servpro-cloud-workflows`](./servpro-cloud-workflows/SKILL.md) | Deploys and syncs Google Cloud infrastructure: Cloud Workflows definitions, Cloud Scheduler cron jobs, Pub/Sub triggers, Cloud Run webhook handler, and Secret Manager config. | Deploying automation infrastructure; syncing event-driven triggers; monitoring active workflow executions. |
| [`servpro-post-dash-notes`](./servpro-post-dash-notes/SKILL.md) | Posts Tristan-approved CRM notes to Dash and sends approved Gmail drafts. Runs **only** after the `tristan-approval` environment gate is cleared. | After Tristan approves the CRM review output. Never before. |
