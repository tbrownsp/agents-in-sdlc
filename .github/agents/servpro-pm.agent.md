...
2. **Maximize the Mon/Wed/Fri Scheduling Pattern via Calendar Automation**: When running the `servpro-ops-review` skill, ask the agent to output a `.ics` formatted block or copy-pasteable details specifically matching your daily standards. Secure your Thursday 1-hour sync with Elizabeth by asking the agent to draft the recurring blocker details for you to add to Google Calendar manually.
