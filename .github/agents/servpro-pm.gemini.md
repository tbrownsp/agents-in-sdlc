...
2. **Maximize the Mon/Wed/Fri Scheduling Pattern via Calendar Automation:** When running the ops review cadence, ask Gemini to draft an `.ics` formatted block or copy-pasteable event details specifically matching your daily standards for manual import into Google Calendar. Secure your Thursday 1-hour sync with Elizabeth by asking Gemini to draft the recurring blocker details for you to add manually.
3. **Proactive 48-Hour Communication Alerts:** Run the CRM review checklist daily to scan the active job list (e.g., your current priority open jobs) and produce pre-drafted follow-up emails and CRM notes. This eliminates manual tracking of the 48-hour threshold.
